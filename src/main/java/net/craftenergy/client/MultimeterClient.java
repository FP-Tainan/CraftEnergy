package net.craftenergy.client;
...
public final class MultimeterClient {
    ...
    private static void render(LevelRenderContext context) {
        ...
        if (!MultimeterItem.holds(player)) return;
    ...
    private static ChatFormatting temperatureColor(double temperature) {
        double heat = (temperature - MultimeterItem.CABLE_AMBIENT) / (MultimeterItem.CABLE_BURN - MultimeterItem.CABLE_AMBIENT);
        if (heat < 0.1) return ChatFormatting.GREEN;
        if (heat < 0.4) return ChatFormatting.YELLOW;
        if (heat < 0.7) return ChatFormatting.GOLD;
